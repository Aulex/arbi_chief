...
class _StreetballCrossTableTabState
  ...
  Widget _buildTotalStandingsView(List<String> groupNames) {
    ...
    void addFromStandings(List<scoring.StreetballStanding> standings, String phase) {
      for (final s in standings) {
        ...
        final scored = cumulative?.pointsScored ?? s.pointsScored;
        final conceded = cumulative?.pointsConceded ?? s.pointsConceded;
        rankedTeams.add((
          teamId: s.teamId,
          teamName: s.teamName,
                  ...
                  child: Table(
                    ...
                    children: [
                      TableRow(
                        ...
                        children: [
                          _summaryHeaderCell('Місце', headerStyle),
                          _summaryHeaderCell('Команда', headerStyle, minWidth: 180),
                          _summaryHeaderCell('Очки', headerStyle),
                          _summaryHeaderCell('М\'ячі', headerStyle),
                          _summaryHeaderCell('Різн.', headerStyle),
                          _summaryHeaderCell('Етап', headerStyle, minWidth: 90),
                        ],
                      ),
                      for (int i = 0; i < rankedTeams.length; i++)
                        TableRow(
                          ...
                          children: [
                            _summaryDataCell(
                              '${rankedTeams[i].overallPlace}',
                              cellStyle,
                              bold: true,
                            ),
                            _summaryDataCell(
                              rankedTeams[i].teamName,
                              cellStyle,
                              leftAlign: true,
                            ),
                            _summaryDataCell(
                              '${rankedTeams[i].matchPoints}',
                              cellStyle,
                              bold: true,
                            ),
                            _summaryDataCell(
                              '${rankedTeams[i].pointsScored}:${rankedTeams[i].pointsConceded}',
                              cellStyle,
                            ),
                            _summaryDataCell(
                              '${rankedTeams[i].pointDiff >= 0 ? '+' : ''}${rankedTeams[i].pointDiff}',
                              cellStyle,
                            ),
                            _summaryDataCell(rankedTeams[i].phase, cellStyle),
                          ],
                        ),
                    ],
                  ),
  ...
  Widget _summaryHeaderCell(String text, TextStyle style, {double minWidth = 48}) {
    return Container(

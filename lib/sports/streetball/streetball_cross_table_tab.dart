...
class StreetballCrossTableTab extends ConsumerStatefulWidget {
  ...
  const StreetballCrossTableTab({super.key, required this.tId, required this.tournamentName});

  @override
  ConsumerState<StreetballCrossTableTab> createState() => _StreetballCrossTableTabState();
}

class _StreetballCrossTableTabState extends ConsumerState<StreetballCrossTableTab> {
  bool _loading = true;
  ...
  List<scoring.StreetballStanding> _calcStandings(List<({int teamId, String teamName, int? teamNumber, int? entityId})> teams) {
  ...
  Future<void> _del(int id) async { await ref.read(streetballServiceProvider).deleteTeamGame(id); await _loadData(); }
  ...
  Future<void> _clearAll() async { final svc = ref.read(streetballServiceProvider); for (final id in _games.values.map((g) => g.eventId).toSet()) { await svc.deleteTeamGame(id); } await _loadData(); }

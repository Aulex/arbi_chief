...
import 'streetball_scoring.dart';
import 'streetball_service.dart';

/// Streetball-specific PDF report builder.
///
...
class StreetballReportBuilder {
  final StreetballService _streetballService;
  final TeamService _teamService;
  final TournamentService _tournamentService;

  StreetballReportBuilder(this._streetballService, this._teamService, this._tournamentService);

  /// Check whether the tournament has any streetball data to report on.
  ...
  Future<pw.Document> buildPdf(Tournament tournament, SportTypeConfig config) async {
    ...
    final rawGames = await _streetballService.getTeamGamesForTournament(tId);
    final groupAssignments = await _streetballService.getGroupAssignments(tId);
    final removedTeamIds = await _streetballService.getRemovedTeamIds(tId);
    ...
    for (final t in teamList) {
      ...
      if (entityId == null) {
        entityId = await _streetballService.ensureTeamEntity(t.teamId);
      }
      teams.add(_TeamInfo(
        teamId: t.teamId,
        teamName: t.teamName,
    ...
    for (final g in rawGames) {
      if (g.eventResult != null) {
        gamesMap[(g.teamAEntityId, g.teamBEntityId)] = g.eventResult!;
  ...
  List<StreetballStanding> _calculateStandings(_BuildContext ctx, List<_TeamInfo> teams) {
  ...
  void _addTotalStandingsPage({
    ...
    void addFromStandings(List<StreetballStanding> standings, String phase) {
      for (final s in standings) {

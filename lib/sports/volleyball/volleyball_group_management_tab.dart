...
/// Group management tab for volleyball tournaments with ≥ 9 teams.
///
/// Displays groups as columns with team names.
/// Supports automatic random draw and manual reassignment.
class VolleyballGroupManagementTab extends ConsumerStatefulWidget {
  final int tId;

  const VolleyballGroupManagementTab({super.key, required this.tId});

  @override
  ConsumerState<VolleyballGroupManagementTab> createState() => _VolleyballGroupManagementTabState();
}

class _VolleyballGroupManagementTabState extends ConsumerState<VolleyballGroupManagementTab> {
  bool _loading = true;
  ...
  Future<void> _autoAssignGroups() async {
    ...
    await vSvc.autoAssignGroups(widget.tId, teamIds);
    await _loadData();
  }
  ...
  Future<void> _clearGroups() async {
    ...
    final vSvc = ref.read(volleyballServiceProvider);
    await vSvc.clearGroupAssignments(widget.tId);
    await _loadData();
  }
  ...
  Future<void> _reassignTeam(int teamId, String currentGroup) async {
    ...
    final vSvc = ref.read(volleyballServiceProvider);
    await vSvc.setGroupAssignment(widget.tId, teamId, newGroup);
    await _loadData();
  }

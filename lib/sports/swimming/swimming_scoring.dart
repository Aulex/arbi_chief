...
List<SwimmingStanding> calculateStandings({
  required List<({int teamId, String teamName})> teams,
  required List<({int teamId, int place})> individualResults,
  int? maxResultsPerTeam = 7, // Swimming counts top 7 results
  Set<int> removedTeamIds = const {},
}) {
  final standings = <int, SwimmingStanding>{};

  for (final team in teams) {
    standings[team.teamId] = SwimmingStanding(
      teamId: team.teamId,

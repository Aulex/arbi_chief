...
class _AthleticsTeamStandingsTabState
  ...
  Widget _buildStandingRow(
    int index,
    AthleticsTeamStanding s,
    Widget Function(Widget, {int? flex, double? width, AlignmentGeometry align}) dataCell,
    TextStyle cellStyle,
  ) {
    final isHovered = _hoveredRow == index;
    final m35 = _formatPlaces(s.categoryPlaces[AthleticsCategory.m35] ?? []);
    final m49 = _formatPlaces(s.categoryPlaces[AthleticsCategory.m49] ?? []);
    final m50 = _formatPlaces(s.categoryPlaces[AthleticsCategory.m50] ?? []);
    final f35 = _formatPlaces(s.categoryPlaces[AthleticsCategory.f35] ?? []);
    final f49 = _formatPlaces(s.categoryPlaces[AthleticsCategory.f49] ?? []);
    final f50 = _formatPlaces(s.categoryPlaces[AthleticsCategory.f50] ?? []);
      ...
      child: Container(
        ...
        child: Row(
          children: [
            ...
            dataCell(Text(f50, style: cellStyle), flex: 1),
            dataCell(
              Text(
